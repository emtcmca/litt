# RUN THE PROTOTYPE & THE LIVE APP — side-by-side fidelity checks

Two things run locally: **the live dashboard** (what you're building, in `dashboard/`) and **the
prototype** (the design target, in `prototype-source/`). Open them side-by-side at the same width
and compare. Do this after each build phase. Read `CURRENT-STATE.md` first (what exists) and
`DIVERGENCES.md` (where the live app intentionally differs from the prototype today).

---

## PART A — Run the live dashboard (the thing you're iterating on)

```bash
# 1. Backend (serves /api, /health) — must be up or pages error
cd backend && <your backend run cmd>          # serves http://localhost:8002

# 2. Dashboard
cd dashboard && npm install && npm run dev     # → http://localhost:3000
```

- Vite is pinned to **port 3000** (`vite.config.ts`) and **proxies `/api` + `/health` to
  `http://localhost:8002`**. If the backend isn't running there, data fetches
  (`/api/brief`, `/api/sweep`, `/api/tools`, `/api/deadlines`, …) fail and pages render empty.
- Demo data: ensure the demo firm is seeded (`POST /api/demo/reset`) and `GET /api/demo/ready`
  passes. The demo date is `2026-06-25` (`ConsoleShell`).
- Routes to visit (no hashes — real paths): `/`, `/deadlines`, `/relationships`, `/collect`,
  `/anomalies`, `/budgets`, `/agents`, `/ledger`, `/policy`, `/integrations`.
- Type/build check before committing: `cd dashboard && npm run build` (runs `tsc -b`).
- Tests: `npm test` (Vitest), `npm run test:e2e` (Playwright).

> The live app is **teal/mint on near-black** today, not the prototype's forest/brass — that's a
> known divergence (`DIVERGENCES.md` §1), not a bug. Compare **layout/structure** first; treat
> color per the palette decision.

---

## PART B — Run the prototype (the design target)
...
## 5. Quick reference — exact timings & counts

**Prototype (design target)** — verified against `prototype-source/console-agents.jsx`:

- Idle heartbeat interval: **2100ms**, cycling the **4 specialist agents only**.
- Sweep step cadence (`STEP_MS`): **640ms** per step. **20** steps total.
- Boundary stat: **17/20 deterministic · 3 Gemini** (computed `total − llm_assisted`).

**Live app (as-built today)** — verified against `dashboard/src/...`, see `CURRENT-STATE.md` §6:

- Idle heartbeat: **900ms**, cycling **7 nodes** (coordinator + 4 agents + audit + brief),
  `AgentGraph.tsx`.
- Playback: **`PLAYBACK_STEP_MS = 220`**, `AgentConsole.tsx`. Steps = live
  `timeline.observations.length` (not a fixed 20).
- Boundary stat lives in the **Inspector** (Log tab), counting
  `obs.data.tool.kind === 'gemini'` over `TOOL_CALL`s.

These differ on purpose-vs-drift — reconcile per `DIVERGENCES.md` §3 once the Agent-console
fidelity target is set.
