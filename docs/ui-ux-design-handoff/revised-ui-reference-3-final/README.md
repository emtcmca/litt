...
> ### ▶ The codebase now has a real Console build. Start with `CURRENT-STATE.md`.
> The backend + dashboard scaffolding are built (all routes, pages, `components/console/*`,
> real `api.ts`/`types.ts`). Before any UI work:
> 1. **`CURRENT-STATE.md`** — the exact as-built map (routes, tokens, data hooks, component
>    APIs, Agent-console internals, dev commands). Build on these real files.
> 2. **`DIVERGENCES.md`** — where the built code differs from the design target, with the
>    decisions to make (palette is the big one). Settle these before pixel work.
> 3. **`RUN-THE-PROTOTYPE.md`** — run the live app (`:3000`) and the prototype side-by-side.
>
> The method below (translate source, match screenshot) still holds for *layout/structure* — but
> the live app is **teal/mint on near-black today, not forest/brass** (`DIVERGENCES.md` §1).
...
ui-reference/
  ...
  CURRENT-STATE.md    ← the as-built Console, exactly (routes, tokens, data, component APIs) — START HERE
  DIVERGENCES.md      ← built code vs design target + the decisions to make
  ...
  RUN-THE-PROTOTYPE.md ← run the live app (:3000) + the prototype, for live side-by-side
  prototype-source/   ← the exact prototype files that render the screenshots (design target)
  screens/            ← rendered PNG of every page + key states (1440px, the visual target)
```

After each build phase, run **both** the live app and the prototype and compare (not just the
PNGs) — `RUN-THE-PROTOTYPE.md` has the commands, the route↔screenshot map, and the timings for
each. `CURRENT-STATE.md` is the source of truth for what's actually in the repo.
