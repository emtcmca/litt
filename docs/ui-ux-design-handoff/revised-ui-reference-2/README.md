...
ui-reference/
  ...
  RUN-THE-PROTOTYPE.md ← serve the prototype locally + drive it for live side-by-side checks
...
After each build phase, run the prototype and compare live (not just against the PNGs) —
`RUN-THE-PROTOTYPE.md` has the serve command, the hash→route→screenshot map, and the exact
animation timings/counts to match.
