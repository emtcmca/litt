...
| `paper` | `#FBFAF6` | *(brand off-white; defined in `T` but the shell does NOT use it for the content bg — see note)* | — | reserved / not the content bg |
...
- **Content:** scrollable, **`background: wash` (`#F1EFE8` / `--color-background-tertiary`)** — set
  on the `<main>` in `console-shell.jsx` (not `paper`; `paper` exists in `T` but is unused for
  the shell bg). Per-page wrappers don't set their own bg, so they inherit `wash`. Inner
  `max-width` ~880 on Overview, ~920–980 on other pages; padding `24–30px ... 60px`.
